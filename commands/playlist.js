	...
	run: async function (client, message, args) {
		...
		if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
			...
			try {
				...
				let thing = new MessageEmbed()
					.setAuthor("Playlist has been added to queue", "https://raw.githubusercontent.com/Hisana1220/mb/main/assets/Music.gif")
		...
		async function handleVideo(video, message, channel, playlist = false) {
			...
			if (!serverQueue) {
				...
				if (playlist) return;
				let thing = new MessageEmbed()
					.setAuthor("Song has been added to queue", "https://raw.githubusercontent.com/Hisana1220/mb/main/assets/Music.gif")
...
async	function play(guild, song) {
  ...
  let afk = JSON.parse(fs.readFileSync("./afk.json", "utf8"));
    ...
    if (!song){
      if (!online.afk) {
        sendError("Oh! Ended queue! Thank you for your using!", message.channel)
